...
//middleware là handler giúp kiểm tra dữ liệu mà ng dùng truyền lên
//có đủ và đúng như nhu cầu định dạng kh

//giờ ta sẽ phát triển chức năng login
//và cần kiểm tra xem email và password đủ kh ?
...
const forgotPasswordSchema: ParamSchema = {
  ...
  custom: {
    options: async (value: string, { req }) => {
      //value lúc này là forgot_password_token thì ktra luôn
      try {
      ...
      Authorization: {
        ...
        custom: {
          options: async (value, { req }) => {
            ...
            //nhma mình cần access_token th
            const access_token = value.split(' ')[1]
            if (!access_token) {
              //client truyền bearer thiếu access_token thì cho nghỉ luôn
              throw new ErrorWithStatus({
                status: HTTP_STATUS.UNAUTHORIZED, //401
            ...
            try {
              ...
              //decode_authorization là payload lấy từ vc verify access token
              // console.log(decode_authorization)
              ;(req as Request).decode_authorization = decode_authorization // cất vào request để qa tầng khác xài
            } catch (error) {
              throw new ErrorWithStatus({
                status: HTTP_STATUS.UNAUTHORIZED, //401
                message: (error as JsonWebTokenError).message
  ...
  checkSchema(
    {
      refresh_token: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            try {
              //nhảy zô verify token luôn | value chính là refresh_token
              const decode_refresh_token = await verifyToken({
                token: value,
                privateKey: process.env.JWT_SECRET_REFRESH_TOKEN as string
              })
              //thành công thì lưu vào request
